...
if [ "$(id -u)" = "0" ]; then
    mkdir -p /app/data
    # The built-in updater atomically replaces /app/sub2api, so the runtime
    # user must be able to create temporary files and rename entries in /app.
    chown sub2api:sub2api /app /app/sub2api 2>/dev/null || true
