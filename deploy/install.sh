...
# Sub2API 安装脚本
# Usage: curl -sSL https://raw.githubusercontent.com/killaragorn/sub2api-pro/product/main/deploy/install.sh | bash
#
...
# Configuration
GITHUB_REPO="killaragorn/sub2api-pro"
...
Documentation=https://github.com/killaragorn/sub2api-pro
...
upgrade() {
    ...
    # Get current version
    CURRENT_VERSION=$("$INSTALL_DIR/sub2api" --version 2>/dev/null | grep -oE 'v?[0-9]+\.[0-9]+\.[0-9]+(-[0-9A-Za-z.-]+)?(\+[0-9A-Za-z.-]+)?' || echo "unknown")
