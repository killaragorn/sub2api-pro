export default {
    ...
    riskControl: {
      ...
      cyberRequestContent: 'System Prompt / 用户输入（已脱敏）',
      cyberRequestSaved: '已保存 Cyber 系统提示词/用户输入快照',
      cyberRequestLoading: '正在读取审计快照…',
      cyberRequestLoadFailed: '读取 Cyber 系统提示词/用户输入快照失败',
      cyberRequestUnavailable: '该记录没有可用的系统提示词/用户输入快照，可能是升级前产生的历史记录。',
      cyberRequestProtocol: '协议',
      cyberRequestSize: '已保存 {stored} / 原始请求 {original} 字节',
      cyberRequestTruncated: '脱敏后的系统提示词/用户输入快照超过 64 KiB，仅保留头部和尾部。',
      cyberUpstreamEvidence: '上游 Cyber 错误证据',
