export default {
    settings: {
      ...
      openaiPrioritySaturation: {
        title: 'OpenAI 优先级饱和调度',
        description: '独立于实验加权调度。按 Priority 升序、账号 ID 升序确定性选取，前一账号的新会话普通区满载后才使用下一账号。',
        enable: '启用优先级饱和调度',
        enableHint: '与实验加权调度互斥；切换策略时会先要求确认，并在一次保存中同时提交两个开关。',
        activeHint: '当前生效：新会话按普通区容量 G 调度；已有 session_hash 和 previous_response_id 可使用包含预留 R 的完整容量 C。',
        orderLabel: '账号顺序',
        orderValue: 'Priority 升序，Priority 相同时按账号 ID 升序',
        generalLabel: '新会话与临时溢出',
        generalValue: '只使用普通容量 G = C - R',
        affinityLabel: '原账号亲和请求',
        affinityValue: '优先使用完整硬上限 C',
        reserveLabel: '保护槽位',
        reserveValue: '逐账号配置亲和预留 R',
        capacityWarning: 'R=0 的账号没有亲和保护；C=0 表示无限并发，排在前面的无限账号不会因容量满载而自然扩散。',
        manageAccounts: '前往账号管理配置 Priority、C 与 R',
        switchToPriorityTitle: '切换到优先级饱和调度？',
        switchToPriorityMessage: '确认后将关闭 TopK 加权调度并启用优先级饱和调度。权值配置会保留，直到再次切回。',
        switchToAdvancedTitle: '切换到实验加权调度？',
        switchToAdvancedMessage: '确认后将关闭优先级饱和调度并启用 TopK 加权调度。账号的亲和预留 R 仍然生效。',
        switchConfirm: '确认切换',
        mutualExclusionError: '优先级饱和调度与实验加权调度不能同时启用，请关闭其中一个后再保存。'
      },
