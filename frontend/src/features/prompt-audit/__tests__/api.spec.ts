...
describe('Prompt Audit API', () => {
  ...
  it('sends a temporary probe token only in the request and never invents response credentials', async () => {
    ...
    const result = await promptAuditAPI.probeEndpoint({
      id: 'guard-1', name: 'Guard', protocol: 'groq_safeguard', base_url: 'https://api.groq.com/openai', model: 'openai/gpt-oss-safeguard-20b',
      token: 'api-canary-secret', clear_token: false, timeout_ms: 1000, input_limit: 1000, tpm_limit: 8000, enabled: true, has_token: false, token_status: 'missing',
    })
    expect(client.post).toHaveBeenCalledWith('/admin/prompt-audit/endpoints/probe', expect.objectContaining({
      endpoint: expect.objectContaining({ protocol: 'groq_safeguard', token: 'api-canary-secret', tpm_limit: 8000 }),
    }))
