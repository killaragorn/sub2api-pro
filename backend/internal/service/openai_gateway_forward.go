...
func (s *OpenAIGatewayService) Forward(ctx context.Context, c *gin.Context, account *Account, body []byte) (*OpenAIForwardResult, error) {
	...
	if account.IsOpenAIOAuth() && responsesLiteRequested {
		...
		// 缺省或其它取值一律显式改写。
		if gjson.GetBytes(body, "parallel_tool_calls").Raw != "false" && s.openAIResponsesRectifierEnabled(ctx) {
	...
	for {
		...
		if resp.StatusCode >= 400 {
			...
			if retryBody, reason, changed, retryErr := normalizeOpenAIResponsesRejectedFieldRetryBody(resp.StatusCode, body, respBody, s.openAIResponsesRectifierEnabled(ctx)); retryErr != nil {
