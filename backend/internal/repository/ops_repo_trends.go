...
SELECT
  ...
  COUNT(*) FILTER (WHERE ` + opsOperationalErrorPredicate("") + `) AS error_total,
  COUNT(*) FILTER (WHERE COALESCE(is_business_limited, false) = true) AS business_limited,
  COUNT(*) FILTER (WHERE COALESCE(status_code, 0) >= 400 AND NOT is_business_limited AND NOT is_sla_excluded) AS error_sla,
  COUNT(*) FILTER (WHERE error_owner = 'provider' AND NOT is_business_limited AND NOT is_sla_excluded AND COALESCE(upstream_status_code, status_code, 0) NOT IN (429, 529)) AS upstream_excl,
  COUNT(*) FILTER (WHERE error_owner = 'provider' AND NOT is_business_limited AND NOT is_sla_excluded AND COALESCE(upstream_status_code, status_code, 0) = 429) AS upstream_429,
  COUNT(*) FILTER (WHERE error_owner = 'provider' AND NOT is_business_limited AND NOT is_sla_excluded AND COALESCE(upstream_status_code, status_code, 0) = 529) AS upstream_529
FROM ops_error_logs
` + where + `
GROUP BY 1
ORDER BY 1 ASC`
...
SELECT
  ...
  COUNT(*) AS total,
  COUNT(*) FILTER (WHERE COALESCE(status_code, 0) >= 400 AND NOT is_business_limited AND NOT is_sla_excluded) AS sla,
