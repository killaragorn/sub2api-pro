...
func (m *ConfigManager) buildNextStorage(current storageConfig, req UpdateConfigRequest, actorID int64) (storageConfig, error) {
	...
	next := storageConfig{
		...
		GroqSafeguardPolicy: canonicalStoredSafeguardPolicy(req.GroqSafeguardPolicy),
	...
	for _, endpoint := range req.Endpoints {
		...
		stored := StorageEndpoint{
			...
			TimeoutMS: endpoint.TimeoutMS, InputLimit: endpoint.InputLimit, TPMLimit: endpoint.TPMLimit, Enabled: endpoint.Enabled,
		}
