...
func (e *Enqueuer) Enqueue(ctx context.Context, req Request) error {
	...
	snapshot, err := ExtractPromptSnapshotForEndpoints(req, endpoints)
	...
	payloadValue, err := encodePromptAuditPayload(snapshot)
	if err != nil {
		e.recordDropped()
	...
	if err := e.payload.Set(ctx, job.ID, payloadValue, DefaultPayloadTTL); err != nil {
